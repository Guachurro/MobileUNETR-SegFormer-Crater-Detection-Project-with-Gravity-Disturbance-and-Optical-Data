...
class SegmentationDatasetCreator(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        crater_img = torch.from_numpy(resized_img)#.repeat(3, 1, 1)  # Repeat grayscale to 3 channels (RGB)
        crater_label = torch.from_numpy(resized_label)  # No need to repeat for label, single channel is fine
...
class SegFormerFinetuner(pl.LightningModule):
    def __init__(self,id2label,train_dataloader=None,val_dataloader=None,test_dataloader=None, metrics_interval=5,modelweights=None,hyperparameters=None):
                 ...
                 #Model Imported from Hugging Face
                 self.model=SegformerForSemanticSegmentation.from_pretrained(self.modelweights, #This determined which weights to bring to the model
                                                                             return_dict=False,
    ...
    def training_step(self, batch, batch_nb):
        ...
        outputs=self(images,masks.squeeze()) 
        ...
        self.train_mean_iou.add_batch(
            ...
            references=masks.squeeze(1).detach().cpu()
            )
    ...
    def validation_step(self, batch, batch_nb):
        ...
        images, masks = batch['pixel_values'], batch['labels']
        outputs=self(images,masks.squeeze()) #Is this step calling the forward function? Yes
        loss, logits=outputs[0], outputs[1]
    ...
    def on_test_epoch_end(self):
        ...
        for i in range(num_images_to_visualize):
            ...
            ax[2].imshow(all_preds[i].cpu().numpy(), cmap='gray')
            ax[2].set_title('Predicted Mask')
            ax[3].imshow(all_preds[i].cpu().numpy(), cmap='gray', alpha=0.5)
...
#You can manually edit this if your training was interrupted for some reason. 
for k in range(Attempts):
    ...
    #1) Model Weights 
    weights1="nvidia/segformer-b0-finetuned-ade-512-512"
    chosenweights=weights1
    
    #2) Randomize Encoder Freezing. 
    freezenc=random.choice([False, True])
    
    #Parameter passed to PyTorch Lightning
    modelinfo=[chosenweights,freezenc]

    #Hyperparameter Information
    batch_size=random.choice([8,16,24,32,40]) #Generally a higher batch size is better. 
    batch_size_eval=64 #Changing this doesnt affect results
    epochs=100 #How many epochs for each model
    ...
    adambetas=(.9,.999)
    amsgrad=random.choice([True,False]) #Only Relevant for AdamW, not Adam. 
    ...
    CheckpointPath=f"F:/Academics/OSU/Thesis Documents/Images and Labels/Dataset {dataset}/ModelCheckpoint_SegFormer/Attempt {Run}/Iteration {tensorversion}/"
    CheckpointPathT=f"F:/Academics/OSU/Thesis Documents/Images and Labels/Dataset {dataset}/ModelCheckpoint_SegFormer/Attempt {Run}"
    Tensorname='SegFormer'
...
#%% Example of how to visualize results using test dataset
#TESTING SegFormer DATASET 1 RUN 19
seed=664393850 #Choose value that was in the same run
random.seed(seed)
np.random.seed(seed)
torch.manual_seed(seed)
imageresize=128
...
#Create DataLoaders
test_dataloader1=DataLoader(test_dataset,batch_size=32)

#Define model and load it with the selected checkpoint
testmodel=SegFormerFinetuner.load_from_checkpoint("Path to Checkpoint")
